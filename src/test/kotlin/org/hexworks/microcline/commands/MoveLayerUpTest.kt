...
@DisplayName("Given a move layer up command")
class MoveLayerUpTest {
    ...
    @DisplayName("When executing it within bounds it should properly move the layer up")
    @Test
    fun shouldProperlySelectWhenWithinBounds() {

        val result = MoveLayerUp(1).execute(snapshot)
    ...
    fun shouldNotMoveAboveBound() {
        ...
        val result = MoveLayerUp(2).execute(snapshot)

        assertThat(result).isEqualTo(snapshot)
    }

    @DisplayName("When executing it when layer is already at top it should return old state")
    @Test
    fun shouldNotMoveAtTopBound() {

        val result = MoveLayerUp(0).execute(snapshot)
